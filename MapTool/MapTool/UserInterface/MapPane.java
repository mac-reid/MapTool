...
public class MapPane {
	...
	// Last grid location of the cursor (while in full menus such as
	// FileChooser)
	int tempGridX = 0;
	int tempGridY = 0;
	...
	public MapPane(String maplocation, int sizex, int sizey, genUI genUI)
	...
	public void dragMap(int x, int y) {
		...
		if (dragoffsetx != 0) {
			...
			if ((pxOffsetX + dragoffsetx) > 0
					&& (pxOffsetX + dragoffsetx) < (pxSizeX - paneSizeX - (pxSizeX % tokenScale)))
		...
		if (dragoffsety != 0) {
			// If drag is within bounds
			if ((pxOffsetY + dragoffsety) > 0
					&& (pxOffsetY + dragoffsety) < (pxSizeY - paneSizeY - (pxSizeY % tokenScale)))
	...
	// Objects drawn in the MapPane should determine render status via their own
	// isActive() method
	public void renderMap(float x, float y, Graphics g) {
		...
		// Otherwise, render Map/Grid/Tokens
			...
			// Otherwise, draw the map grid and tokens based on the mouse-drag
			// offset
			else {
				...
				drawGrid(x, y, g);
				tokens.renderTokens(x, y, pxOffsetX, pxOffsetY, pxOffsetX
						+ paneSizeX, pxOffsetY + paneSizeY, genUI.getIcons());
			}
	 ...
	 */
	public void update(int mXoffset, int mYoffset, int mapXsize, int mapYsize,
			GameContainer gc, int delta) throws SlickException {
		...
		if (tempList.size() > 0) {
			...
			for (String s : tempList) {
				String[] data = s.split("~");
				tokens.addToken(data[1], Integer.parseInt(data[2]),
		...
		if (fileChooser.isActive()) {
			...
			if (!(fileChooser.getSelected().equals(""))) {
				if (loadMode == 2)
					addTokenGrid(fileChooser.getSelected(), tempGridX,
							tempGridY);
				else if (loadMode == 1)
		...
		// If the Menus are not being used, a Drag is possibly taking place
			...
			if ((mouseX >= mXoffset && mouseX <= mXoffset + paneSizeX)
					&& (mouseY >= mYoffset && mouseY <= mYoffset + paneSizeY)) {
				...
				if (in.isMouseButtonDown(0)) {
					...
					if (dragMode == 0) {
						if (tokens.isOccupied(currentGridX, currentGridY)) {
							...
							dragImage = tokens.getImage(currentGridX,
									currentGridY).copy();
							dragImage.setAlpha(0.75f);
							// set the selected token
							selectedToken = tokens.getToken(dragToken.x,
									dragToken.y);
						}
				...
				// If LeftMouse is not down, but a state is set, clear the state
				// appropriately
				else if (dragMode == 1) {
					dragMode = 0;
				} else if (dragMode == 2) {
					// Indicates the mouse was released from a token drag.
					// Attempt to move the token
					tokens.moveToken((int) dragToken.getX(),
							(int) dragToken.getY(), currentGridX, currentGridY);

					// added by Mac
					try {
						genUI.control.moveToken((int) dragToken.getX(),
								(int) dragToken.getY(), currentGridX,
								currentGridY);
				...
				// Modes have not been set, menus are not in use - addition
				// click functions (right/middle) go here
				...
				// [MouseWheel Polling] - Used to scale and orient the grid (via
	...
	// Increments/Decrements the Grid/Token scale (used in conjunction with live
	// mousewheel input)
	...
	// Sets SwingFileChooser active, accepts mouse coordinates where the
	// resource should be placed
	...
	public void addTokenCoord(String imglocation, int x, int y)
			throws SlickException {
		this.addTokenGrid(imglocation, (int) ((x + pxOffsetX) / tokenScale),
				(int) ((y + pxOffsetY) / tokenScale));
	...
	public void addTokenGrid(String imglocation, int x, int y)
			throws SlickException {
	...
	public void removeTokenCoord(int x, int y) {
		this.removeTokenGrid((int) ((x + pxOffsetX) / tokenScale),
				(int) ((y + pxOffsetY) / tokenScale));
	}
	...
	private void drawGrid(float getX, float getY, Graphics g) {
		for (float x = 0; x < pxSizeX; x += tokenScale)
			g.drawLine(getX + x - pxOffsetX, getY - pxOffsetY, getX + x
					- pxOffsetX, getY - pxOffsetY + pxSizeY);

		for (float y = 0; y < pxSizeY; y += tokenScale)
			g.drawLine(getX - pxOffsetX, getY + y - pxOffsetY, getX + pxOffsetX
					+ pxSizeX, getY + y - pxOffsetY);
	}
